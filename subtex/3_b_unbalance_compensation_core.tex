...
\subsection{Problem statement}\label{VUB:sec:Statement}
    ...
    \subsection{Optimization based control algorithm}\label{VUB:sec:Optimization}
        ...
        %\KwResult{Write here the result }
         $\textbf{x}_i^0=0$, $\Delta_i^0=0$\;
         \While{$f_{grid}(\textbf{x}_i^{q}+\Delta_i^*d_i)\neq0$}{
          evaluate $f_{grid}(\textbf{x}_i^{(q)}+\Delta_i^{(q)}d_i)$\;
          \eIf{$f_{grid}(\textbf{x}_i^{(q)}+\Delta_i^{(q)}d_i)<f_{grid}(\textbf{x}_i^{(q)})$}{
          $\textbf{x}_i^{(q+1)}=\textbf{x}_i^{(q)}+\Delta_i^{(q)}d_i$\;
         ...
         \label{algo:APPS}
        \end{algorithm}
        where f_{grid} representing the multi variate voltage response function of the grid of which VU cost function could be calculated, based on current injection. The function arguments are $\textbf{x}_i$, the actuator for the $i^{th}$ process aka. the amplitude or phase of the three phase current phasor, needs to be applied on the system, for VU reduction, $\Delta_i$ is the process step lenght aka. the value of the current vector's amplitude or angle needs to be changed for a successful step, and $d_i$ is the corresponding step's signed direction vector, which specifies the applied changes direction. The algorithm is initialised with $\textbf{x}_i^0=0$, $\Delta_i^0=0$ for a smooth start, due to lack of prior network knowledge.\\
